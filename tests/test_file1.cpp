...
TEST_CASE("Test Case 1: Testing formatting and reading.") 
{
    ...
    std::vector<double>arr1 = c.calculate(" ; ;; ;;; ;;;; ;;;;;"); // reads all prints and returns <>
    CHECK(arr1.size() == 0);

    std::vector<double>arr2 = c.calculate("q"); // should close program and return <>
    CHECK(arr2.size() == 0);
    ...
    std::vector<double>arr4 = c.calculate(" 1; 1;; 1;;;q 1;;;; q;;;;;"); // should return <1,1,1> 
    CHECK(equalDouble(arr4[0], 1) == true);
    CHECK(equalDouble(arr4[1], 1) == true);
    CHECK(equalDouble(arr4[2], 1) == true);
    CHECK(arr4.size() == 3);

    std::vector<double>arr5 = c.calculate("1;;;;;;;;;;;;;;;;;;;;;;;;;;;;q"); // should return <1> 
    CHECK(equalDouble(arr5[0], 1) == true);
    CHECK(arr5.size() == 1);
    
    std::vector<double>arr6 = c.calculate("q 25; 24+2; 300%10; 26/0; 234-2;"); // should return <> 
    CHECK(arr6.size() == 0);
...
TEST_CASE("Test Case 6: Testing error messages.") 
{
    ...
    arr1 = c.calculate("(23-23d); (23-24); (26-6);");
    CHECK(calc_err == CPError::MISSINGPARENTH);
    CHECK(equalDouble(arr1[0], -1) == true);
    CHECK(equalDouble(arr1[1], 20) == true);
    CHECK(arr1.size() == 2);
